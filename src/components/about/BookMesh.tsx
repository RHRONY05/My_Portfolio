...
export function BookMesh({
  ...
  // Textures for covers and all 4 inner page spreads
  ...
  // Leaf 1 turns when going to chapter 2, 3, or 4 (~ -136.8 deg / -2.39 rad)
  const targetLeaf1Angle = isOpen && chapter >= 2 ? -Math.PI * 0.76 : 0;
  // Leaf 2 turns when going to chapter 3 or 4 (~ -131.4 deg / -2.29 rad)
  const targetLeaf2Angle = isOpen && chapter >= 3 ? -Math.PI * 0.73 : 0;
  // Leaf 3 turns when going to chapter 4 (~ -126.0 deg / -2.20 rad)
  ...
  return (
    <group ref={rootGroupRef} position={[0, 0, 0]}>
      ...
      {/* 4. BASE RIGHT PAGE (Chapter 04 Right: School Artwork & Story) */}
      <group position={[-width / 2, 0, pagesBlockThickness / 2]}>
        ...
        {page4RightTexture && (
          <mesh position={[pagesWidth / 2, 0, 0.002]}>
            <planeGeometry args={[pagesWidth, pagesHeight]} />
            <meshStandardMaterial
      ...
      {/* 5. LEAF 3: Front = Chapter 03 Right, Back = Chapter 04 Left */}
      <group
        ref={leaf3Ref}
        position={[-width / 2, 0, pagesBlockThickness / 2]}
      >
        <mesh position={[pagesWidth / 2, 0, 0.004]}>
        ...
        {page3RightTexture && (
          <mesh
            position={[pagesWidth / 2, 0, 0.0048]}
            onClick={(e) => {
              e.stopPropagation();
              if (isOpen && chapter === 3) {
                onChapterChange?.(4);
        ...
        {page4LeftTexture && (
          <mesh
            position={[pagesWidth / 2, 0, 0.0032]}
            rotation={[0, Math.PI, 0]}
            onClick={(e) => {
              e.stopPropagation();
              if (isOpen && chapter === 4) {
      ...
      {/* 6. LEAF 2: Front = Chapter 02 Right, Back = Chapter 03 Left */}
      <group
        ref={leaf2Ref}
        position={[-width / 2, 0, pagesBlockThickness / 2]}
      >
        <mesh position={[pagesWidth / 2, 0, 0.007]}>
          <boxGeometry args={[pagesWidth, pagesHeight, 0.001]} />
          <meshStandardMaterial color="#08080A" roughness={0.8} />
        </mesh>
        ...
        {page2RightTexture && (
          <mesh
            position={[pagesWidth / 2, 0, 0.0078]}
            onClick={(e) => {
              e.stopPropagation();
              if (isOpen && chapter === 2) {
                onChapterChange?.(3);
        ...
        {page3LeftTexture && (
          <mesh
            position={[pagesWidth / 2, 0, 0.0062]}
            rotation={[0, Math.PI, 0]}
            onClick={(e) => {
              e.stopPropagation();
              if (isOpen && chapter === 3) {
      ...
      {/* 7. LEAF 1: Front = Chapter 01 Right, Back = Chapter 02 Left */}
      <group
        ref={leaf1Ref}
        position={[-width / 2, 0, pagesBlockThickness / 2]}
      >
        <mesh position={[pagesWidth / 2, 0, 0.01]}>
          <boxGeometry args={[pagesWidth, pagesHeight, 0.001]} />
          <meshStandardMaterial color="#08080A" roughness={0.8} />
        </mesh>
        ...
        {page1RightTexture && (
          <mesh
            position={[pagesWidth / 2, 0, 0.0108]}
            onClick={(e) => {
              e.stopPropagation();
              if (isOpen && chapter === 1) {
                onChapterChange?.(2);
